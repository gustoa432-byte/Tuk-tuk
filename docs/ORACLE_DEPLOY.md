...
>
> Internet Gateway — **опциональная** часть: приложение работает офлайн по BLE/DTN без аккаунта и без сервера. Эндпойнты `oracle_*` — легаси/эксперимент, продуктовым UI не используются. Что видит оператор сервера: [`PRIVACY.md`](PRIVACY.md).
